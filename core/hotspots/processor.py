...
5. **严禁结构性标签**：正文中不得出现"事件钩子""拆解博弈""技术逻辑""预判观点""互动收尾"等结构性标签文字。文章是给读者看的，不是大纲。
...
# 文章结构 (5 段式，严禁输出"事件钩子""拆解博弈"等结构性标签文字)
1. 开篇（200 字内）：用一个最新的具体事件切入——某条新闻、某次声明、某项数据。直接点明"这件事为什么重要"。
2. 多角度分析（600-800 字）：围绕事件展开多角度分析。问自己：谁在受益？谁在受损？背后的权力/利益格局是什么？用"这背后的算盘是......""选在这个节点绝非偶然"来展开。
3. 技术与产业深挖（600-800 字）：从技术壁垒、产业链重构、政策意图三个维度深挖。引用具体数据、政策文件、历史案例。用 **加粗** 标出关键数据和核心论点。
...
4. **重点分级标注**（严格遵守，全文红色标注不超过 5 处）：
   - **红色加粗**：仅用于全文最核心的结论、颠覆性判断、关键数据。用 `**{{重点}}**` 标记（花括号包裹）。
   - **黑色加粗**：用于重要论点、关键概念、人物/公司名。用 `**加粗**` 标记（无花括号）。
   - 普通正文：不加任何标记。
   判断标准：如果一个读者只看红色文字，能抓住全文最精华的一句话——那就是红色。
