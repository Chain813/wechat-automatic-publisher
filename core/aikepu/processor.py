...
## 写作要求
1. 文章总字数 3000-5000 字（这是硬性要求！不要偷懒写短了）
2. 每个 ## 小节至少 400 字，深入展开而非一笔带过
3. 必须包含 2-3 个图表占位符【此处绘制图表：描述】
4. 必须包含至少 3 个 > 引用段落（要点回顾、金句或注意事项）
5. 必须包含至少 4 个 ## 二级标题
...
请直接输出完整的 Markdown 格式文章，不要有任何前缀解释。"""
    ...
    if word_count < 2000:
        logger.warning("⚠️ 文章字数不足 2000 (当前 {})，内容偏短", word_count)
    if word_count > 6000:
        logger.warning("⚠️ 文章字数超过 6000，微信可能截断")
