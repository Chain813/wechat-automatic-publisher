...
## 3. 逐步深入（6000-9000 字）
用至少 6 个 ## 小节，从浅到深，每个小节 800-1500 字。
- 每个小节：讲清一个问题，给出一个代码示例，画一张图
- 每个概念至少用 3 个具体例子从不同角度演示
- 必须包含：为什么需要、之前怎么做、现在怎么做、什么场景不适用
- 在关键处插入图表占位符【此处绘制图表：描述】

## 4. 代码实战（500-800 字）
给出 2-3 段 Python 伪代码，让读者对实现有直观感受。代码必须带详细注释。

## 5. 常见误区与进阶思考（400-600 字）
纠正 3-4 个最常见的理解误区。引用论文或业界实践作为依据。

## 6. 核心要点回顾（300-400 字）
用 > 引用格式，列出 6-8 个最重要的要点。配延伸阅读推荐（论文、博客、视频）。
...
## 写作要求（严格遵守！）
1. **文章总字数 8000-12000 字，低于 7000 字视为严重不合格。**
   - 这是一篇可以收藏、打印、反复研读的深度教程。不是快餐。
   - 每个概念五步展开：直觉 → 原理 → 数学直觉 → 代码示意 → 应用场景 → 局限与前沿。
2. 至少 6 个 ## 二级标题，每个小节 800-1500 字
3. 必须包含 4-5 个图表占位符【此处绘制图表：描述】
4. 必须包含至少 5 个 > 引用段落（金句、要点、注意事项）
5. 至少 3 处用具体代码片段（Python 伪代码，不超过 10 行）辅助讲解
6. 至少引用 2 篇经典论文或知名博客作为延伸阅读
...
请直接输出完整的 Markdown 格式文章，不要有任何前缀解释。"""
    ...
    if word_count < 5000:
        logger.warning("⚠️ 文章字数严重不足 (当前 {})，远低于 8000 字要求", word_count)
    elif word_count < 7000:
        logger.warning("⚠️ 文章字数偏短 (当前 {})，未达 7000 字最低要求", word_count)
